...
function encodeCommand(name) {
  ...
  if (code === undefined) throw new RangeError(`Ukjent LINAK-sengekommando: ${name}`);
...
function guessAddressType(mac) {
  ...
  if (!Number.isInteger(msb)) throw new TypeError(`Ugyldig MAC: ${mac}`);
...
function resolveHandles(services) {
  ...
  if (!command) throw new Error('Fant ikke LINAKs kommandokarakteristikk 99fa0002 — er dette en LINAK-seng?');
