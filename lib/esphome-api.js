...
function encodeVarint(value) {
  let x = BigInt(value);
  if (x < 0n) throw new RangeError('varint kan ikke være negativ');
  const out = [];
  do {
...
function decodeVarint(buf, offset) {
  ...
  while (pos < buf.length) {
    ...
    shift += 7n;
    if (shift > 70n) throw new Error('varint er for lang');
  }
...
class EsphomeApiClient extends EventEmitter {
  constructor({ host, port = 6053, clientInfo = 'homey-linak-bed', log = () => {} } = {}) {
    super();
    if (!host) throw new TypeError('EsphomeApiClient krever host');
    this.host = host;
    this.port = port;
  ...
  connect({ timeoutMs = 10000 } = {}) {
    return new Promise((resolve, reject) => {
      ...
      const timer = setTimeout(() => {
        socket.destroy();
        reject(new Error(`ESPHome API: tidsavbrudd mot ${this.host}:${this.port}`));
      }, timeoutMs);
      ...
      socket.on('close', () => {
        ...
        this._failAll(new Error('ESPHome API: forbindelsen ble lukket'));
        this.emit('close');
      });
  ...
  waitForDevice(mac, { timeoutMs = 15000 } = {}) {
    ...
    return new Promise((resolve, reject) => {
      ...
      const timer = setTimeout(() => {
        finish(reject, new Error(`ESPHome API: så ikke ${mac} innen ${timeoutMs} ms`));
      }, timeoutMs);
      this._acquireAdvertisements();
    });
  }
    ...
    return new Promise((resolve, reject) => {
      const off = this._collect((type, fields) => {
        ...
        if (Number(fields[2] || 0) === 1) {
          ...
          reject(new Error(`BLE-tilkobling avvist (error=${Number(fields[4] || 0)})`));
        }
      });
      const timer = setTimeout(() => {
        off();
        reject(new Error(`BLE-tilkobling: tidsavbrudd etter ${timeoutMs} ms`));
      }, timeoutMs);
  ...
  blePair(mac, { timeoutMs = 15000 } = {}) {
    ...
    return new Promise((resolve, reject) => {
      const off = this._collect((type, fields) => {
        ...
        else reject(new Error(`BLE-bonding avvist (error=${Number(fields[3] || 0)})`));
      });
      const timer = setTimeout(() => {
        off();
        reject(new Error(`BLE-bonding: tidsavbrudd etter ${timeoutMs} ms`));
      }, timeoutMs);
  ...
  getServices(mac, { timeoutMs = 20000 } = {}) {
    ...
    return new Promise((resolve, reject) => {
      ...
      const timer = setTimeout(() => {
        off();
        reject(new Error('GATT: tidsavbrudd på tjenesteoppdagelse'));
      }, timeoutMs);
  ...
  writeRaw(frame) {
    if (!this._socket || this._socket.destroyed) {
      throw new Error('ESPHome API: ikke tilkoblet');
    }
  ...
  gattWrite(mac, handle, data, { withResponse = true, timeoutMs = 8000 } = {}) {
    ...
    return new Promise((resolve, reject) => {
      ...
      const timer = setTimeout(() => {
        off();
        reject(new Error(`GATT-skriving på handle ${handle}: tidsavbrudd`));
      }, timeoutMs);
      this._send(MSG.GATT_WRITE_REQ, payload);
    });
  }
  ...
  _gattError(fields) {
    ...
    const err = new Error(`GATT-feil på handle ${Number(fields[2] || 0)}: error=${att}`);
    err.code = 'GATT_ERROR';
    err.attError = att;
    return err;
  }
  ...
  _request(type, payload, expectType, timeoutMs) {
    return new Promise((resolve, reject) => {
      ...
      const timer = setTimeout(() => {
        off();
        reject(new Error(`ESPHome API: tidsavbrudd på meldingstype ${expectType}`));
      }, timeoutMs);
  ...
  _onData(chunk) {
    ...
    for (;;) {
      ...
      if (this._rx[0] !== 0x00) {
        ...
        this._failAll(new Error('ESPHome API: forventet plaintext-ramme (er kryptering slått på?)'));
        this._socket.destroy();
        return;
      }
